...
function testCreateClassroomAssignment() { 
  ...
  // --- NEW: Variables to track our summary data ---
  let postedCount = 0;
  let errorCount = 0;
  let postedTitles = [];
  ...
  for (let i = 1; i < data.length; i++) { 
    ...
    if ( status !== "PENDING" || !isReadyToPost(postDateRaw, today) || !courseId || courseId === "ENTER_YOUR_COURSE_ID_HERE" ) { 
      continue; 
    }
    ...
    try { 
      ...
      // --- NEW: Track success for LINE summary ---
      postedCount++;
      ...
      // --- NEW: Track error for LINE summary ---
      errorCount++;
    } 
  }
  
  // --- NEW: Send the LINE summary if anything was processed ---
  if (postedCount > 0 || errorCount > 0) {
    let summaryText = "📚 Classroom Automation Run\n\n";
    summaryText += `✅ Posted: ${postedCount}\n`;
    
    if (errorCount > 0) {
      summaryText += `❌ Errors: ${errorCount}\n`;
    }
    
    if (postedTitles.length > 0) {
      summaryText += `\n📝 Assignments Created:\n- ${postedTitles.join('\n- ')}`;
    }
    
    sendLineSummary(summaryText);
  }
}

// ==========================================
// --- NEW: LINE API Function ---
// ==========================================
function sendLineSummary(summaryText) {
  // Replace these with your actual keys from the LINE Developer Console
  const CHANNEL_ACCESS_TOKEN = 'YOUR_LONG_CHANNEL_ACCESS_TOKEN_HERE';
  const USER_ID = 'YOUR_USER_ID_STARTING_WITH_U_HERE'; 
  ...
  try {
    ...
    Logger.log("Failed to send message to LINE: " + error);
  }
}
...
function parsePoints(rawPoints) { 
  ...
  if (!pointsStr || pointsStr === "ungraded" || pointsStr === "0") { return 0; }
  const points = Number(rawPoints); 
  if (Number.isNaN(points) || points < 0) { throw new Error("Invalid points value: " + rawPoints); }
  return points;
}
...
function getOrCreateTopicId(courseId, topicName, topicCache) { 
  if (!topicCache[courseId]) { 
    ...
    topicCache[courseId] = {};
    topics.forEach(topic => { topicCache[courseId][topic.name] = topic.topicId; }); 
  }
  if (topicCache[courseId][topicName]) { return topicCache[courseId][topicName]; }
...
function buildMaterials(materialUrl) { 
  const driveIdMatch = materialUrl.match(/\/d\/([a-zA-Z0-9-_]+)/) || materialUrl.match(/[?&]id=([a-zA-Z0-9-_]+)/);
  if (driveIdMatch && driveIdMatch[1]) { 
    const fileId = driveIdMatch[1];
    try { DriveApp.getFileById(fileId); } 
    catch (e) { throw new Error("Cannot access Drive file. You must own it or have edit access to make student copies."); }
    return [{ driveFile: { driveFile: { id: fileId }, shareMode: "STUDENT_COPY" } }]; 
  }
  return [{ link: { url: materialUrl } }];
}
